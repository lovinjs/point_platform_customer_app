...
登录后首页会显示“账单记录”入口。`pages/transactions/index` 分为积分明细、充值记录和消费记录三个标签，支持下拉刷新及触底分页；进入页面时同步刷新积分余额。超级管理员登记整笔充值退款后，客户端会显示负向退款流水和“已退款”的原充值订单。客户端不展示平台收款参考号、后台操作员或幂等键等内部字段。
